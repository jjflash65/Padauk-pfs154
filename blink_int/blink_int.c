...
volatile uint8_t ticks;                        // wird durch den Interrupthandler fortgezaehlt
...
void t16_init(void)
{
  // F_CPU / 1 / 256 = 8 MHz / 256 = 31.25 kHz
  // Interruptausloesung alle 0.032 ms

//   T16M= 0x80;
  T16M = (uint8_t)(T16M_CLK_IHRC | T16M_CLK_DIV1 | T16M_INTSRC_8BIT);

  T16C = 0;                     // Timerzaehlregister = 0;
  ticks = 0;
  millis = 0;
...
void main(void)
{
  ...
  while(1)
  {
    while (millis< 500);          // ca. Sekundentakt (2,4% Fehler)
    led_toggle();
    millis= 0;
  }
}
