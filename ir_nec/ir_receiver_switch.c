...
void main(void)
{
  ...
  while(1)
  {
    ...
    if (ir_newflag)
    {
      ...
      switch (ir_code)
      {
        case (0x0000) : sr_value ^= MASK0; sr_setvalue(sr_value); break;  // Kanal-0
        case (0x0008) : sr_value ^= MASK1; sr_setvalue(sr_value); break;  // Kanal-1
        case (0x0009) : sr_value ^= MASK2; sr_setvalue(sr_value); break;  // Kanal-2
        case (0x000A) : sr_value ^= MASK3; sr_setvalue(sr_value); break;  // Kanal-3
        case (0x000C) : sr_value ^= MASK4; sr_setvalue(sr_value); break;  // Kanal-4
        case (0x000E) : sr_value ^= MASK5; sr_setvalue(sr_value); break;  // Kanal-5
        case (0x0014) : sr_value ^= MASK6; sr_setvalue(sr_value); break;  // Kanal-6
        case (0x0016) : sr_value ^= MASK7; sr_setvalue(sr_value); break;  // Kanal-7
